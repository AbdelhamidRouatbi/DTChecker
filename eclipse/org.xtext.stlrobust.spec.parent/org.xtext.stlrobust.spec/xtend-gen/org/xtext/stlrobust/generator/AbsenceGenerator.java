...
public class AbsenceGenerator extends SpecGenerator {
  public static Object generate(final Specification spec) {
    ...
    if ((_scope_1 instanceof GlobalScope)) {
      _matched=true;
      return AbsenceGenerator.genGlobalScope(spec);
    }
    if (!_matched) {
      Scope _scope_2 = spec.getScope();
      if ((_scope_2 instanceof BeforeScope)) {
        _matched=true;
        return AbsenceGenerator.genBeforeScope(spec);
      }
    }
    if (!_matched) {
      ...
      if ((_scope_3 instanceof AfterScope)) {
        _matched=true;
        return AbsenceGenerator.genAfterScope(spec);
      }
    }
    if (!_matched) {
      ...
      if ((_scope_4 instanceof BetweenScope)) {
        _matched=true;
        return AbsenceGenerator.genBetweenScope(spec);
      }
    }
    if (!_matched) {
      ...
      if ((_scope_5 instanceof AfterUntilScope)) {
        _matched=true;
        return AbsenceGenerator.genAfterUntilScope(spec);
      }
    }
  ...
  public static String genGlobalScope(final Specification spec) {
    ...
    Absence pattern = ((Absence) _pattern);
    String p = SpecGenerator.expression(pattern.getP());
    StringConcatenation _builder = new StringConcatenation();
    _builder.append("always[0,0](not");
    _builder.append(p);
    _builder.append(")");
