  ...
  def static time(Time t){
		if (t !== null){
			...
			switch(t.unit){
				...
				case "min": multiplier = 1000 * 60
			}
			
			switch(t){
			case t instanceof UpperTimeBound: return '''[0, «(t as UpperTimeBound).value*multiplier»]'''
			case t instanceof LowerTimeBound: return '''[«(t as LowerTimeBound).value*multiplier»,«(t as LowerTimeBound).value*multiplier»]'''
			case t instanceof Interval: return '''[«(t as Interval).min*multiplier»,«(t as Interval).max*multiplier»]'''
			} 
		}
		return ''''''
	}
	...
	def static expression(Expression e){
		...
		val operator = if (e.operator == "=") "==" else e.operator
